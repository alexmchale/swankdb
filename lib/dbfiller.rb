...
class Array
  def randsub
    map {|e| e if Kernel.rand < 0.5}.compact
  end
end

def self.loremof(length)
  while true
    words ||= [ 1, words.to_i ].max
    result = lorem(words).simple
    return result if result.length == length
    words -= 1 if result.length > length
    words += 1 if result.length < length
  end
end
...
tagwords = lorem(50).simple.split
...
count.times do |i|
  ...
  entry = Entry.new
  entry.user = user
  entry.content = lorem(Kernel.rand 500)
  entry.tags = tagwords.randsub.join(' ')
  entry.save
end
